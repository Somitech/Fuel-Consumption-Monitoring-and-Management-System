    ...
    public partial class InsertCounter : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (this.textBox1.Text != "" && this.textBox2.Text != "" && this.textBox4.Text != "")
            {
                ...
                // إدخال العداد
                string query1 = "Select Count(*) as [CounterNum] from Counter where CarID = " + carID;
                ds1 = db.select(query1);
                dr = ds1.Tables[0].Rows[0];
                int CounterNum = Int32.Parse(dr["CounterNum"].ToString());
                ...
                query1 = "Select Count(*) as [PreviosCounter] from Counter where CarID = " + carID + " and Month = " + month + " and Year = " + year;
                ds1 = db.select(query1);
                dr = ds1.Tables[0].Rows[0];
                int previosCounter = Int32.Parse(dr["PreviosCounter"].ToString());



                if (previosCounter == 0)
                {
                    MessageBox.Show("عداد الشهر السابق لم يتم إدخاله \n الرجاء إدخال عداد الشهر السابق والمتابعة");
                }
                else if(previosCounter != 0)
                {
                    ...
                    query1 = "select * from Counter where CarID = " + carID;
                    ds1 = db.select(query1);
                    dt1 = ds1.Tables[0];
                    ...
                    int beginCounter, endCounter, distance, consumingRate, defaultFuel, realFuel, monthlyExcess, preComulativeExcess, comulativeExcess;
